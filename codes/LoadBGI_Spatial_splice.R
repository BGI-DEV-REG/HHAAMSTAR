...
LoadBGI_Spatial_splice <- function(filename,
                            outdir = getwd(),
                            bin_data = F,
                            bin_size = 50,
                            cell_mask = T,
  ...
  print('mark')}
    ...
    colnames(dat)[1:5] <- c("geneID", "x", "y","count","UMICount")
    if (bin_data) {
        rowname_style <- "BIN"
    ...
    if (cell_mask) {
        ...
        rowname_style <- "CELL"
        colnames(dat)[1:6] <- c("geneID", "x", "y", "UMICount", "ExonCount", "label")
        if (delete_bg){
	dat<-subset(dat,label>0)}
        ...
        dat_bkp <- dat
        dat_bkp <- dat_bkp[, sum(ExonCount), by = .(geneID, x, y)]
        ...
        dat <- dat[, sum(ExonCount), by = .(geneID, x, y,label)]
